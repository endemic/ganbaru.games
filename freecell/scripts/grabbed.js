...
export default class Grabbed extends Stack {
  ...
  overlaps(stack) {
    // Calculate the sides of the bounding boxes
    let left1 = stack.x;
    let right1 = stack.x + stack.size.width;
    let top1 = stack.y;
    let bottom1 = stack.y + stack.size.height;

    let left2 = this.x + this.size.width / 4;
    let right2 = this.x + this.size.width * 3 / 4;
    let top2 = this.y + this.size.height / 4;
    let bottom2 = this.y + this.size.height * 3 / 4;
